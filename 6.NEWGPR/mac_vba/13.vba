Sub CreateHierarchyTreeWithHeader()
    ...
    ReDim colsToSum(0 To 26)
    colsToSum(0) = 7   ' G
    colsToSum(1) = 16  ' P
    colsToSum(2) = 17  ' Q
    j = 3
    For colNum = 23 To 46 ' W to AT
        colsToSum(j) = colNum
        j = j + 1
    Next colNum
        ...
        For Each keyE In dict(keyB).Keys
            ...
            For Each keyF In dict(keyB)(keyE).Keys
                ...
                For Each keyG In dict(keyB)(keyE)(keyF).Keys
                    ...
                    ' Внедрение локальной формулы для диапазона W:AT (столбцы 23-46)
    ...
    MsgBox "Структура с формулами распределения по месяцам создана!", vbInformation
End Sub
