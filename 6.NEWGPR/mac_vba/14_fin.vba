Sub CreateHierarchyTreeWithHeader()
    ...
    Dim colsToSum() As Variant, cIdx As Variant, colNum As Long
    ...
    destWs.Cells(6, 1).Value = "№"
    destWs.Cells(6, 2).Value = "Объект"
    destWs.Range(destWs.Cells(6, 3), destWs.Cells(6, colCount + 2)).Value = _
        srcWs.Range(srcWs.Cells(6, 8), srcWs.Cells(6, maxCol)).Value
        
    destWs.Cells(7, 1).Value = "№"
    destWs.Cells(7, 2).Value = "Объект"
    destWs.Range(destWs.Cells(7, 3), destWs.Cells(7, colCount + 2)).Value = _
        srcWs.Range(srcWs.Cells(7, 8), srcWs.Cells(7, maxCol)).Value
        
    With destWs.Range(destWs.Cells(6, 1), destWs.Cells(7, colCount + 2))
        .Font.Bold = True
        .Font.Color = RGB(255, 255, 255)
        .Interior.Color = RGB(52, 73, 94)
        .VerticalAlignment = xlCenter
    End With
    destWs.Rows(6).RowHeight = 25
    destWs.Rows(7).RowHeight = 25
    
    ReDim colsToSum(0 To 50)
    colsToSum(0) = 7
    colsToSum(1) = 16
    colsToSum(2) = 17
    j = 3
    ...
    For i = LBound(keysArr) To UBound(keysArr)
        ...
        For Each keyE In dict(keyB).Keys
            ...
            For Each keyF In dict(keyB)(keyE).Keys
                ...
                For Each keyG In dict(keyB)(keyE)(keyF).Keys
                    ...
                    destWs.Range(destWs.Cells(r, 23), destWs.Cells(r, 46)).FormulaLocal = _
                        "=ЕСЛИ(W$7>=$T" & r & ";ЕСЛИ(КОНМЕСЯЦА(W$7;0)<=$U" & r & ";($R" & r & ")/($U" & r & "-$T" & r & "+1)*ДЕНЬ(КОНМЕСЯЦА(W$7;0));ЕСЛИ(КОНМЕСЯЦА(W$7;0)=КОНМЕСЯЦА($U" & r & ";0);($R" & r & ")/($U" & r & "-$T" & r & "+1)*($U" & r & "-W$7+1);0));ЕСЛИ(КОНМЕСЯЦА(W$7;0)>=$T" & r & ";ЕСЛИ(КОНМЕСЯЦА(W$7;0)>=$U" & r & ";$R" & r & ";$R" & r & "/($U" & r & "-$T" & r & "+1)*(КОНМЕСЯЦА(W$7;0)-$T" & r & "+1));0))"
                        
                    destWs.Range(destWs.Cells(r, 229), destWs.Cells(r, 252)).FormulaR1C1 = _
                        "=IF(R7C>=RC20,IF(EOMONTH(R7C,0)<=RC21,(RC15)/(RC21-RC20+1)*DAY(EOMONTH(R7C,0)),IF(EOMONTH(R7C,0)=EOMONTH(RC21,0),(RC15)/(RC21-RC20+1)*(RC21-R7C+1),0)),IF(EOMONTH(R7C,0)>=RC20,IF(EOMONTH(R7C,0)>=RC21,RC15,RC15/(RC21-RC20+1)*(EOMONTH(R7C,0)-RC20+1)),0))"
                ...
                If r - 1 > startR3 Then
                    For Each cIdx In colsToSum
                        destWs.Cells(startR3, CInt(cIdx)).FormulaR1C1 = "=SUM(R[" & (startR3 + 1 - startR3) & "]C:R[" & (r - 1 - startR3) & "]C)"
                    Next cIdx
                End If
            Next keyF
            ...
            If r - 1 > startR2 Then
                For Each cIdx In colsToSum
                    destWs.Cells(startR2, CInt(cIdx)).FormulaR1C1 = "=SUMIF(R[" & (startR2 + 1 - startR2) & "]C:R[" & (r - 1 - startR2) & "]C,""<>"",R[" & (startR2 + 1 - startR2) & "]C:R[" & (r - 1 - startR2) & "]C)/2"
                Next cIdx
            End If
        Next keyE
        ...
        If r - 1 > startR1 Then
            For Each cIdx In colsToSum
                destWs.Cells(startR1, CInt(cIdx)).FormulaR1C1 = "=SUMIF(R[" & (startR1 + 1 - startR1) & "]C:R[" & (r - 1 - startR1) & "]C,""<>"",R[" & (startR1 + 1 - startR1) & "]C:R[" & (r - 1 - startR1) & "]C)/3"
            Next cIdx
        End If
    Next i
    
    destWs.Columns("T:U").NumberFormat = "dd.mm.yyyy"
    
    For Each cIdx In colsToSum
        destWs.Columns(CInt(cIdx)).NumberFormat = "#,##0.00"
    Next cIdx
    
    destWs.Columns.AutoFit
    destWs.Range("C1,H1,M1,S1,V1,AU1").EntireColumn.ColumnWidth = 4
    
    With destWs.Columns("B:B")
        .ColumnWidth = 90
        .WrapText = True
    End With
    ...
    MsgBox "Макрос полностью завершил генерацию структуры!", vbInformation
End Sub
