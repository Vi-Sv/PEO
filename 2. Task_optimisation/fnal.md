...
### Прогнозирование рисков: Аномалия множественных технологических пробелов

**Суть проблемы:** Если выгрузка содержит подряд 3-4 и более пустых строк внутри живой сметы, текущий «Двойной чек-поинт» может ложно определить это как конец таблицы и преждевременно срезать данные.

**Решение (Сканер пустоты):** Замена жесткого условия `i + 1` на динамический цикл, который активирует «нож» только после обнаружения критической массы пустых строк подряд (например, 5 или 10).

#### Резервный блок кода (для вставки в макрос):

```vba
    ' --- СКАНЕР МАССИВА ПУСТОТЫ ---
    Dim blankCounter As Long, requiredBlanks As Long
    requiredBlanks = 5 ' лимит пустоты
    firstBlankRow = 0
    blankCounter = 0
    ...
    For i = 2 To lastRow
        If Trim(ws.Cells(i, "D").Value) = "" And Trim(ws.Cells(i, "E").Value) = "" Then
            ...
            If blankCounter = requiredBlanks Then
                firstBlankRow = (i - requiredBlanks + 1)
                Exit For
            End If
        Else
            blankCounter = 0 ' Сброс при встрече данных
        End If
    Next i
...
Этот код превращает макрос в интеллектуальный радар, игнорирующий технологические карманы, но точно удаляющий монолитный подвал.
